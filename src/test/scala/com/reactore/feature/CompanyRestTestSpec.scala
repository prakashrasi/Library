...
class CompanyRestTestSpec extends WordSpec with Matchers with ScalatestRouteTest {
   ...
   "Company Rest" should {
      "return a company list" in {
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
      ...
      "throw exception in get all for empty company list" in {
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.emptyList)
         Get("/company") ~> testRoute ~> check {
      ...
      "insert a company to database" in {
         val newCompany = Company(4, "RENAULT", licenceNumber = "REN001", countryId = 2,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
      ...
      "throw exception if country list is empty" in {
         val newCompany = Company(4, "RENAULT", licenceNumber = "REN001", countryId = 2,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.emptyList)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
         when(MockCompanyService.companyRepository.insert(any[Company])).thenReturn(Future.successful(1))
         Post("/company").withEntity(newCompany) ~> testRoute ~> check {
            status shouldEqual StatusCodes.BadRequest
      ...
      "throw exception in insert company if name and licence number not defined" in {
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
         val newCompany = Company(4, "", licenceNumber = "", countryId = 3,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         Post("/company").withEntity(newCompany) ~> testRoute ~> check {
            status shouldEqual StatusCodes.BadRequest
         }
      }
      "throw exception in insert company if country does not exists" in {
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
         val newCompany = Company(4, "RENAULT", licenceNumber = "REN001", countryId = 4,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         Post("/company").withEntity(newCompany) ~> testRoute ~> check {
            status shouldEqual StatusCodes.BadRequest
         }
      }
      "throw exception for insert company if duplicate country is inserted" in {
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
         val newCompany = Company(1, "TATA", licenceNumber = "TA001", countryId = 3,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         Post("/company").withEntity(newCompany) ~> testRoute ~> check {
            status shouldEqual StatusCodes.BadRequest
         }
      }
      ...
      "delete company for company id as 3" in {
         when(MockCompanyService.vehicleRepository.getAllVehicles).thenReturn(MockVehicleRepository.vehicleFuture)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
      ...
      "update company details" in {
         val updatedCompany = Company(2, "Scania", licenceNumber = "SCAN001", countryId = 2,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
      ...
      "throw exception company id as 5" in {
         val updatedCompany = Company(5, "Scania", licenceNumber = "SCAN001", countryId = 2,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
      ...
      "throw exception if country is not found" in {
         val updatedCompany = Company(2, "Scania", licenceNumber = "SCAN001", countryId = 5,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
         Put("/company/2").withEntity(updatedCompany) ~> testRoute ~> check {
            status shouldEqual StatusCodes.BadRequest
      ...
      "throw exception in update company if name and licence number not defined" in {
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         val updatedCompany = Company(1, "", licenceNumber = "", countryId = 1,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
      ...
      "throw exception in update company for duplicate company details" in {
         when(MockCompanyService.companyRepository.getAllCompany).thenReturn(MockCompanyRepository.companyFuture)
         when(MockCompanyService.countryRepository.getAllCountry).thenReturn(MockCountryRepository.countryFuture)
         val updatedCompany = Company(1, "TATA", licenceNumber = "TA001", countryId = 1,startYear = Timestamp.valueOf("1970-01-01 00:00:00")).asJson
         Put("/company/1").withEntity(updatedCompany) ~> testRoute ~> check {
            status shouldEqual StatusCodes.BadRequest
         }
      }
